...
test('W22D1 credential loader contract remains text-only and does not import runtime, secret, provider, network, or filesystem behavior', () => {
  ...
  const forbiddenPatterns = [
    ...
    [/from\s+['"]fs\/promises['"]/u, 'credential loader contract must not import fs/promises'],
    [/from\s+['"]node:fs['"]/u, 'credential loader contract must not import node:fs'],
    [/from\s+['"](?:node:)?child_process['"]/u, 'credential loader contract must not import child_process'],
    ...
    [/\bhttps?\.\s*(?:request|get)\s*\(/u, 'credential loader contract must not call http or https'],
