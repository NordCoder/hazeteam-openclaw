...
export type OcaPresentationDescriptor =
  ...
  | OcaOutputSummaryCardDescriptor
  | OcaDiffSummaryCardDescriptor
...
export interface OcaOutputSummaryCardDescriptor {
...
export interface OcaDiffSummaryCardDescriptor {
...
const URL_MARKER = fromCharCodes([58, 47, 47]);
const BACKSLASH_MARKER = fromCharCodes([92]);
...
export function buildOcaOutputSummaryCard(input: OcaOutputSummaryCardInput): OcaPresentationResult<OcaOutputSummaryCardDescriptor> {
  if (!isRecord(input)) {
  ...
  const card = Object.freeze({
    ...
    ...optionalSessionRef(sessionRef.value),
    artifactRefs: Object.freeze(artifactRefs.value),
    actions: Object.freeze(actions.value),
  } satisfies OcaOutputSummaryCardDescriptor);

  return finishPresentation(card, 'outputSummary');
}

export function buildOcaDiffSummaryCard(input: OcaDiffSummaryCardInput): OcaPresentationResult<OcaDiffSummaryCardDescriptor> {
  if (!isRecord(input)) {
  ...
  const card = Object.freeze({
    ...
    actions: Object.freeze(actions.value),
  } satisfies OcaDiffSummaryCardDescriptor);
...
function optionalSessionRef(value: OcaSessionRef | undefined): { readonly sessionRef?: OcaSessionRef } {
  return value === undefined ? Object.freeze({}) : Object.freeze({ sessionRef: value });
}
...
function isSafeSerializableValue(value: unknown): boolean {
  ...
  if (isRecord(value)) {
    if (isSafeOcaSessionModelJson(value) === false && Object.keys(value).length === 0) {
      return true;
    }
...
function hasUnsafeSummaryMarker(value: string): boolean {
  return (
    ...
    value.includes('\t') ||
    value.includes('/') ||
    value.includes(BACKSLASH_MARKER) ||
    value.includes('..') ||
    value.startsWith('{') ||
    value.startsWith('[') ||
    value.includes('{') ||
